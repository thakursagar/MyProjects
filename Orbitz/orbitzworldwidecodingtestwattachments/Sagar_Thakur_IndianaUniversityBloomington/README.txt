...
The reason for choosing HashMap as the data structure is that HashMap has very good access time of O(1). 
Task 1:
...
pathcost from source in the Node as pathCost and the destination here is the parent of that node. So String is the node itself and destination is the node's parent. 
I have also used an ArrayList<String> which holds the unvisited nodes.
Now, I start evaluating from the source until the destination is reached taking care of the minimum pathcost using the above three data structures. 
